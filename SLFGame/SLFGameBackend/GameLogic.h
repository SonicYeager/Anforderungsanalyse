...
public:
	...
	virtual void	CheckGameFinished	(Event<const std::string&, const Letters&> onNextRound, Event<const std::map<int, PlayerStats>&> onFinalScores) = 0;
	virtual void	HandleGameState(const STATE& state, Event<const std::string&, const Letters&> onSetupRound, Event<const std::map<int, PlayerStats>&> onFinalScores, Event<> onNewGame, Event<GameState> onStandart) = 0;
